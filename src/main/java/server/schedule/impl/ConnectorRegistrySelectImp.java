...
public class ConnectorRegistrySelectImp implements ConnectorRegistry
{
                        ...
                        while (it.hasNext())
                        {
                            ...
                            try
                            {
                                if (key.isValid())
                                {
                                    ...
                                    inputHandlerPool.execute(new MessageHandleThread(key, connector));
                                }
    ...
    static class MessageHandleThread implements Runnable{

        private SelectionKey key;
        private Connector connector;

        public MessageHandleThread(SelectionKey key, Connector connector)
        {
            this.key = key;
            this.connector = (Connector) key.attachment();
        }
    ...
    public void registerInput(Connector connector)
    {
        synchronized (inRegisterInput)
        {
            ...
            try
            {
                ...
                readSelect.wakeup();
                SelectionKey key = channel.register(readSelect, SelectionKey.OP_READ, connector);
